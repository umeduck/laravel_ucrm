...
<template>
  ...
  <div v-if="data.type=='decile'" class="lg:w-2/3 w-full mx-auto overflow-auto">
    <table class="table-auto w-full text-left whitespace-no-wrap">
      ...
      <tbody>
        <tr v-for="data in data.data" :key="data.date">
          <td class="border-t-2 border-b-2 border-gray-200 px-4 py-3">{{ data.decile  }}</td>
          <td class="border-t-2 border-b-2 border-gray-200 px-4 py-3">{{ data.average }}</td>
          <td class="border-t-2 border-b-2 border-gray-200 px-4 py-3">{{ data.totalPerGroup }}</td>
          <td class="border-t-2 border-b-2 border-gray-200 px-4 py-3">{{ data.totalRatio }}</td>
        </tr>
      </tbody>
    </table>
  </div>
  <div v-if="data.type=='rfm'" class="lg:w-2/3 w-full mx-auto overflow-auto">
    ...
    <h3 class="my-4 text-2xl text-center">RFMランク毎の人数</h3>
    <table class="table-auto w-full text-left whitespace-no-wrap">
      <thead>
        <tr>
    ...
    <h3 class="my-4 text-2xl text-center">RとFの集計表</h3>
    <table class="table-auto w-full text-left whitespace-no-wrap">
      <thead>
        <tr>
